...
export default function Options() {
  ...
  useEffect(() => {
    chrome.storage.local.get(
      ["fontSize", "textColor", "bgOpacity", "language", "userEmail"],
      (result) => {
  ...
  const handleSave = () => {
    chrome.storage.local.set({ fontSize, textColor, bgOpacity, language })
    setSaved(true)
  ...
  const handleClearCache = () => {
    chrome.storage.local.get(null, (items) => {
      const transcriptKeys = Object.keys(items).filter((k) => k.startsWith("transcript:"))
      ...
      <div className="max-w-[560px] mx-auto px-space-8 py-space-8">
        ...
        <section className="mb-space-8">
          ...
          <div className="bg-surface border border-border rounded-xl p-space-6">
            <label className="text-label text-text-secondary block mb-space-2">Default language</label>
            <select
              value={language}
              onChange={(e) => setLanguage(e.target.value)}
              className="w-full h-[38px] bg-surface-raised border border-border rounded-sm px-space-3 text-body text-on-surface focus:outline-none focus:border-accent transition-colors"
            >

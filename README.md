...
| `apps/server` | `.env.example` | Deepgram key (required), DeepL key (translation), Supabase URL/anon key (auth gate — verifies tokens via `auth.getUser`, so it works regardless of the project's JWT signing setup; omit to run open), `MAX_CONCURRENT_CONNECTIONS`/`MAX_CONCURRENT_CONNECTIONS_PER_USER` (optional — default 50/3, bounds cost from a bug or abuse against the metered Deepgram/DeepL APIs) |
